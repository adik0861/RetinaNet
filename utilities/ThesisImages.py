...
a1 = np.histogram(cats, bins=list(range(len(coco.cats)+1)))
a2 = a1[0] / len(cats)
d = {labels[i]: a2[i] for i in range(len(coco.cats))}
a3 = np.arange(len(d))
...
for image in coco.imgToAnns.values():
    ...
    _perImageCounts = {k: 0 for k in range(len(coco.cats))}
    _perImageHist = {k: 0 for k in range(len(coco.cats))}
...
allcats = [labels2[x] for x in list(range(len(coco.cats)))]
